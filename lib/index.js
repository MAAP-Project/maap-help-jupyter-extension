...
import { CondaEnvironments, CondaEnvWidget, condaIcon, CONDA_WIDGET_CLASS, IEnvironmentManager } from '@mamba-org/gator-common';
...
async function activateCondaEnv(app, settingsRegistry, palette, menu, restorer) {
    ...
    const settings = await (settingsRegistry === null || settingsRegistry === void 0 ? void 0 : settingsRegistry.load(CONDAENVID));
    const model = new CondaEnvironments(settings);
    // Request listing available package as quickly as possible
    if ((_a = settings === null || settings === void 0 ? void 0 : settings.get('backgroundCaching').composite) !== null && _a !== void 0 ? _a : true) {
...
(function (Private) {
    function loadPackages(model) {
        ...
        setTimeout(() => {
            if (!packageFound) {
                INotification.inProgress('Loading the available packages list in background...').then(id => {
                    toastId = id;
                });
                displayMessage(messages.pop());
            }
